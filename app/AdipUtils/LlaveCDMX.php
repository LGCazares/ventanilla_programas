...
final class LlaveCDMX{
    ...
    /**
     * Indicador para la app si usa o no SSO
     * @var bool
     */
    private $ssoEnabled;
    ...
    public function __construct(){
            ...
            $this->logoutURI = config('llave.logout');
            $this->ssoEnabled = (bool)config('llave.sso');
            ...
            if(strlen(trim($this->logoutURI))==0)    throw new LlaveException('No se definió URL para cerrar sesión en LlaveCDMX');
    ...
    /**
     * Intenta cerrar sesión en el SSO de Llave
     * 
     * @param String $token
     * @return Object
     * @throws LlaveException
     */
    public function logout(String $token):Object{

        if(strlen(trim($token))==0){
            Logg::log(__METHOD__,'No se envió ningún token al método logout()', 500);
            throw new LlaveException('No se envió ningún token al método logout()');
        }
        $my_cURL = new SimpleCURL;
        $my_cURL->setUrl($this->logoutURI);
        $my_cURL->addHeader(['name'=>'Content-Type','value'=>'application/json']);
        $my_cURL->addHeader(['name'=>'accessToken','value'=>''.$token]);
        $my_cURL->useAuthBasic($this->authUser,$this->authPassword);
        $my_cURL->prepare();
        $resultToken = $my_cURL->execute();
        //Log::info('SSO Logout: '.$resultToken);
        $oResult = json_decode($resultToken);
        if(NULL===$oResult){
            Logg::log(__METHOD__,'LlaveCDMX devolvió un objeto vacío a la hora cerrar sesión en SSO', 0);
            die('<h3>Logout failed (LlaveCDMX/SSO)</h3>');
        }
        return $oResult;
