...
final class SimpleCURL{
	...
	/**
	 * Establece el puerto por el que se realiza la conexión
	 * 
	 * @param int $port
	 */
	...
	/**
	 * Hace que el Data se envie "as is" (body en peticiones GET)
	 * 
	 * @param void
	 */
	public function disableQueryStringPayload(){
		$this->queryStringPayload = FALSE;
	}
	...
	public function prepare(){
		...
		if(FALSE===$this->cURL_executor){
			...
			if($this->method == 'GET'){
				if($this->queryStringPayload){ // enviar como querystring
					$this->data =
				...
				}else{ // enviar body en el GET "as is"
					curl_setopt($this->cURL_executor, CURLOPT_URL, $this->url);
					curl_setopt($this->cURL_executor, CURLOPT_CUSTOMREQUEST, "GET");
		...
		// HTTP 1.1
		curl_setopt($this->cURL_executor, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_1);

		// Puerto
		if($this->port !== NULL){
			curl_setopt($this->cURL_executor, CURLOPT_PORT, $this->port);
		}
	...
	public function execute(){
		if($this->isPrepared()){
			...
			if($ret  === FALSE){
				$l = Logg::log(__METHOD__,$this.' '.curl_error($this->cURL_executor), 502);
				abort(502, 'No se obtuvo respuesta del servicio web o esta es incorrecta. Id de incidencia: '.$l);
			}
		}else{
	...
	/**
	 * Obtener info de respuesta
	 * 
	 * @return array $ret
	 */
	public function getResponseInfo(){
		$ret = $this->responseInfo;
		return $ret;
	}
