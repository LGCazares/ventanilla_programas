    ...
    <div class="col-12">
        <div class="d-flex flex-column flex-md-row justify-content-center">
            ...
            <div class="col-12 col-md-6 d-flex flex-column border-left-thin-grey justify-content-center pl-md-4 pl-lg-5">
                <h3 class="error-title mt-5">{{ strlen(trim($exception->getMessage()))==0?'Petición errónea':$exception->getMessage()}}.</h3>
                <p class="grey-light">La petición no es válida</p>
                <small><strong>ID de incidencia:</strong> {{session()->get('requuid')??'N/A'}}</small>
                <div class="align-self-lg-end mt-3 form">
                    <button type="button" onclick="location.href='{{route('welcome')}}';" class="btn-normal">Regresar al inicio</button>
                </div>
            </div>
        </div>
    </div>
